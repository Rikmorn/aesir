...
Each package that has its own concerns carries a `CLAUDE.md` you read first when working there. A Task subagent picks one up on its own, on its first read of a file in that package -- not at dispatch. Sidekick's `sk-executor` does not: it reads a fixed list of paths (`./CLAUDE.md`, `./.claude/rules/*.md`, `./.sidekick/decisions/*.md`) and never opens a nested file on its own. Since the root `CLAUDE.md` is now a symlink to this file, a session started inside a package sees this pointer block too -- something no earlier layout achieved. Even so, the task that sends a worker into a package should name that package's `CLAUDE.md`; if it didn't, read it anyway:
