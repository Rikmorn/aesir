...
## Context
Before v2.3 folded the router into the single agent service, dispatcher code defaulted to `router:3006` when no explicit URL was configured. `git show 39c7015c:.planning/PROJECT.md`'s "Known Tech Debt" list, updated after v2.8, records 22 remaining occurrences of that fallback. They're dead defaults rather than live bugs — the runtime URL resolves correctly via `docker-compose.yml` — but the fallback still points at a service that no longer exists.
...
- `git show 39c7015c:.planning/PROJECT.md` (Known Tech Debt, updated after v2.8)
