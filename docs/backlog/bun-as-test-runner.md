...
## Context
The 2026-09 reset already moved bun in as the runtime for scripts, replacing tsx. The test suite still runs on vitest, and switching it too would touch 49 of the repo's 73 test files, which use `vi.mock`, `vi.fn` or `vi.spyOn`, plus msw and testcontainers-backed integration tests — none of which `bun test` supports as a drop-in replacement.
