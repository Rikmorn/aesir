...
must_haves:
  ...
  key_links:
    - from: "src/api/webhooks/linear-agent-session.ts"
      ...
      pattern: "startWorkflow|WorkflowClient"
...
<task type="auto">
  <name>Task 1: Create Linear AgentSession webhook handler</name>
  <files>
    src/api/webhooks/linear-agent-session.ts
  </files>
  <action>
**Gap 3 Fix: Linear webhook handler for Dev Agent**
...
Note: This requires `startApprovalWorkflow` to be exported from `src/temporal/client.ts`. Check if it exists, if not, add it (it should use `WorkflowClient.start` with `prApprovalWorkflow`).
  </action>
  <verify>
...
<task type="auto">
  <name>Task 2: Add startApprovalWorkflow to Temporal client if missing</name>
  <files>
    src/temporal/client.ts
  </files>
  <action>
...
export async function startApprovalWorkflow(
  ...
  return client.start(prApprovalWorkflow, {
    workflowId,
...
If the function already exists in some form, ensure it matches this signature and behavior.
  ...
  <done>
    ...
    - Function starts prApprovalWorkflow with correct parameters
  </done>
</task>
...
3. Webhook handler exists and is wired:
   - `ls src/api/webhooks/linear-agent-session.ts`
   - `grep -n "linearWebhookHandler" src/scripts/start-dev-agent.ts`
4. Dev Agent starts: `npm run dev-agent` (Ctrl+C to stop)
5. Webhook responds: `curl -X POST http://localhost:3001/webhooks/linear` returns 401
</verification>

<success_criteria>
- Linear AgentSession webhook handler exists at src/api/webhooks/linear-agent-session.ts
- startApprovalWorkflow function exists in Temporal client
- Dev Agent entry point starts Temporal worker and HTTP server
