...
```
  ...
  <verify>
Verify MCP section is present and complete:
```bash
grep -q "MCP Layer" .claude/CLAUDE.md && echo "MCP Layer section exists"
grep -q "3001/mcp" .claude/CLAUDE.md && echo "Port documentation exists"
grep -q "get_issue" .claude/CLAUDE.md && echo "Tool documentation exists"
grep -q "X-Agent-ID" .claude/CLAUDE.md && echo "Header documentation exists"
```
All four grep commands should succeed (exit code 0).
  </verify>
