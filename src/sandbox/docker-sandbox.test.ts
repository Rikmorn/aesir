...
describe("DockerSandbox", () => {
  ...
  describe("execute", () => {
    ...
    it(
      ...
      async () => {
        const result = await sandbox.execute(["sh", "-c", "echo line1; echo line2; echo line3"])
        expect(result.exitCode).toBe(0)
    ...
    it(
      ...
      async () => {
        const result = await sandbox.execute(["node", "-e", "console.log(1 + 2)"])
        expect(result.exitCode).toBe(0)
  ...
  describe("runTests", () => {
    ...
    it(
      ...
      async () => {
        const result = await sandbox.runTests(["sh", "-c", "echo 'Test output'; exit 0"])
        expect(result.passed).toBe(true)
    ...
    it(
      ...
      async () => {
        const result = await sandbox.runTests(["sh", "-c", "echo 'Error!' >&2; exit 1"])
        expect(result.passed).toBe(false)
  ...
  describe("cleanup", () => {
    ...
    it(
      ...
      async () => {
        const sandbox = await DockerSandbox.create()
        await sandbox.cleanup()
        await expect(sandbox.writeFile("/tmp/test.txt", "content")).rejects.toThrow(
          "Sandbox has been cleaned up"
        )
      },
      TEST_TIMEOUT
