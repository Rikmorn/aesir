/**
 * GitHub Credential Store Integration Tests
 *
...
describe("GitHubCredentialStore Integration", () => {
  ...
  afterEach(async () => {
    ...
    if (testOwners.length > 0) {
      await containerCtx.sql.unsafe(
        `
        DELETE FROM github.credentials
  ...
  // Helper to track owners for cleanup
  function trackOwner(owner: string): string {
    testOwners.push(owner);
    return owner;
  }

  // Create a fresh store for each test
  function createStore(): GitHubCredentialStore {
  ...
  describe("store()", () => {
    it("should store a new credential", async () => {
      ...
      const owner = trackOwner("test-org-store-1");
    ...
    it("should soft-delete existing credential when storing new one", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-store-2");
      ...
      // Store second credential for same owner
      ...
      const current = await store.getByOwner(owner);
    ...
    it("should encrypt tokens before storage", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-store-3");
    ...
    it("should store installation_id for GitHub Apps", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-store-4");
      ...
      const retrieved = await store.get(result.value);
      expect(retrieved.isOk()).toBe(true);
      if (retrieved.isOk()) {
        expect(retrieved.value?.installationId).toBe("12345678");
      }
    });
  });
  ...
  describe("get()", () => {
    ...
    it("should retrieve stored credential by ID", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-get-1");
    ...
    it("should not retrieve soft-deleted credentials", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-get-2");
  ...
  describe("getByOwner()", () => {
    it("should return null for owner with no credentials", async () => {
      const store = createStore();

      const result = await store.getByOwner("nonexistent-org");
    ...
    it("should retrieve credential by owner", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-getbyowner-1");
      const testToken = "owner_specific_token";
      ...
      const result = await store.getByOwner(owner);
  ...
  describe("updateTokens()", () => {
    it("should update access token", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-update-1");
    ...
    it("should update refresh token and expiry", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-update-2");
  ...
  describe("delete()", () => {
    ...
    it("should soft-delete existing credential", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-delete-1");
    ...
    it("should return false when deleting already-deleted credential", async () => {
      const store = createStore();
      const owner = trackOwner("test-org-delete-2");
  ...
  describe("test isolation verification", () => {
    it("first test creates owner data", async () => {
      const store = createStore();
      const owner = trackOwner("isolation-test-org");
    ...
    it("second test cannot see first test data", async () => {
      ...
      // This owner should have been cleaned up after the first test
      const result = await store.getByOwner("isolation-test-org");
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        expect(result.value).toBeNull();
      }
