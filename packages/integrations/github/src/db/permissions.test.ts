/**
 * Permission Checker Tests for GitHub Integration
 *
 * Tests the checkGitHubToolPermission function with mocked database.
...
import { checkGitHubToolPermission } from "./permissions.js";

describe("checkGitHubToolPermission", () => {
  ...
  it("should return false when permission exists with allowed=false", async () => {
    ...
    const result = await checkGitHubToolPermission(
      // biome-ignore lint/suspicious/noExplicitAny: Mock database and logger
      { db: mockDb as any, logger: mockLogger as any },
      { agentId: "dev-agent", toolName: "force_push" },
    );
  ...
  it("should return false when no permission row exists (default deny)", async () => {
    ...
    const result = await checkGitHubToolPermission(
      // biome-ignore lint/suspicious/noExplicitAny: Mock database and logger
      { db: mockDb as any, logger: mockLogger as any },
      { agentId: "unknown-agent", toolName: "create_pr" },
    );
