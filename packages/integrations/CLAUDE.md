...
- Webhooks are normalised into `NormalizedEvent` and posted to the agent service's `POST /events`; echo filtering (the service's own actor) happens here, not in the router.
