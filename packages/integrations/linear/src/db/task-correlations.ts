 ...
 *
 * Record and look up correlations between v2.5 task IDs and Linear resources.
 * Used bidirectionally:
 * - Record: When an agent creates a Linear issue/comment via MCP, associate it with the task
 * - Lookup: When a Linear webhook arrives, find the originating task for routing context
 *
...
/**
 * Record a correlation between a task ID and an external Linear resource.
 *
 ...
 * @param externalType - Resource type (e.g., "issue", "comment")
 * @param externalRef - Resource ID (e.g., Linear issue UUID)
...
export async function recordTaskCorrelation(
  ...
  try {
    await db.execute(
      sql`INSERT INTO linear.task_correlations (external_type, external_ref, task_id)
...
/**
 * Look up the task ID associated with an external Linear resource.
 *
...
export async function lookupTaskCorrelation(
  ...
  try {
    const rows = await db.execute(
      sql`SELECT task_id FROM linear.task_correlations
