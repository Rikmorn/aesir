...
// ---------------------------------------------------------------------------
// v2.3 System Prompt (Conversation Terminology)
// ---------------------------------------------------------------------------

/**
 * v2.3 system prompt for the router's LLM classification path.
 *
 * Adapted from ROUTER_SYSTEM_PROMPT with these changes:
 * - "workflow" -> "conversation" for agent execution references
 * - Tool names: start_conversation, signal_conversation, query_conversations
 * - Signal types: domain-language (approval, pr_review, pr_merged, etc.)
 * - Same classification logic, constraints, and structure
 *
 * Both ROUTER_SYSTEM_PROMPT and ROUTER_SYSTEM_PROMPT_V2 coexist until
 * Phase 47 cleanup.
 */
export const ROUTER_SYSTEM_PROMPT_V2 = `You are the Aesir Smart Router. Your job is to classify incoming events and determine the correct routing action.
