...
describe.skip("LEGACY: Product Agent Workflow — Phase 47 cleanup", () => {
  // ---------------------------------------------------------------------------
  // Test Fixtures
  // ---------------------------------------------------------------------------
  ...
  beforeEach(() => {
    ...
    mockSendSlackReplyActivity.mockResolvedValue({
      success: true,
      ts: "ts_123",
    });
  ...
  describe("Query handler", () => {
    it("conversationStatusQuery returns current state", async () => {
      ...
      const queryHandler = capturedHandlers.get(
        "mockQuery_conversationStatus",
